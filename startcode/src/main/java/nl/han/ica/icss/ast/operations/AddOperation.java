...
public class AddOperation extends Operation {
    ...
    public String validate(ExpressionType[] types) {
        if (!types[0].equals(ExpressionType.PERCENTAGE) && !types[0].equals(ExpressionType.PIXEL) && !types[0].equals(ExpressionType.SCALAR)) {
            return "Addition is only allowed with numeric values, not with " + types[0] + ".";
            ...
            return "Addition is only allowed with numeric values, not with " + types[1] + ".";
        } else if (!types[0].equals(types[1])) {
            return "Addition is only allowed with two values of the same type.";
        }
