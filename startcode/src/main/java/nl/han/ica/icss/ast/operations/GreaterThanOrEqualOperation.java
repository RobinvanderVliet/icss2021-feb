...
public class GreaterThanOrEqualOperation extends Operation {
    ...
    public String validate(ExpressionType[] types) {
        if (!types[0].equals(ExpressionType.PERCENTAGE) && !types[0].equals(ExpressionType.PIXEL) && !types[0].equals(ExpressionType.SCALAR)) {
            return "Greater than or equal is only allowed with numeric values, not with " + types[0] + ".";
            ...
            return "Greater than or equal is only allowed with numeric values, not with " + types[1] + ".";
        } else if (!types[0].equals(types[1])) {
            return "Greater than or equal is only allowed with two values of the same type.";
        }
