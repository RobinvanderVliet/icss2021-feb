...
public class MultiplyOperation extends Operation {
    ...
    public NumberLiteral calculate() {
        ...
        if (operands[0] instanceof ScalarLiteral && operands[1] instanceof ScalarLiteral) {
            return new ScalarLiteral(value);
        } else if (operands[0] instanceof ScalarLiteral) {
            if (operands[1] instanceof PercentageLiteral) {
                return new PercentageLiteral(value);
            } else if (operands[1] instanceof PixelLiteral) {
                return new PixelLiteral(value);
            }
        } else if (operands[1] instanceof ScalarLiteral) {
    ...
    public String validate(ExpressionType[] types) {
        if (!types[0].equals(ExpressionType.PERCENTAGE) && !types[0].equals(ExpressionType.PIXEL) && !types[0].equals(ExpressionType.SCALAR)) {
            return "Multiplication is only allowed with numeric values, not with " + types[0] + ".";
            ...
            return "Multiplication is only allowed with numeric values, not with " + types[1] + ".";
        } else if (!types[0].equals(ExpressionType.SCALAR) && !types[1].equals(ExpressionType.SCALAR)) {
            return "Multiplication is only allowed when one of the two values is scalar.";
        }
